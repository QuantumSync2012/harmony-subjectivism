...
**位置づけ**: v0.1.1では『常用字解』を基に初稿・引用を作成した。v0.2.0では著者が『字通』を主軸に再調査した。現在の主たる辞書的基盤は『字通』である。ただし、字ごとの公開証跡には整備済みのものと未整備のものがあり、引用の文言はv0.1.1初稿の『常用字解』（平凡社）由来を保持する。要約・短い引用・出典明示の形で行う（【字源】）。体系との照応はすべて【象徴的読解】であり、論証ではなく布置の読みである。
...
- 本マップの「本体系の読解」列はすべて【象徴的読解】。字源の記述（【字源】）はv0.1.1で白川静『常用字解』から作成した。v0.2.0の再調査は『字通』を主軸とし、字ごとの再確認状態は照合証跡に即して扱う。白川説は出発点として採用する一学説である（→docs/positioning.md）。
