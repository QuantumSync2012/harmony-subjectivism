# 漢字相関マップ — 調和的主観主義 v1.1.0

**多層の枠組み（v0.2.0）**: 本マップの記述は三層に分かれる。①体系内定義（本体系が与える定義。字源から演繹されたものではない）②資料別の字源claim（白川静『字通』『常用字解』等、資料ごとに帰属を明示する）③象徴的読解（体系と字源の響き合い。論証ではない）。以下の表は主として②白川『常用字解』のclaimと③の読みを示すv0.1.1由来の歴史層であり、複数資料の照合結果ではない。資料横断の検証例は[鼎銘と鼎の多層用法](kanji/ding/dingming-preqin-han.md)（`HS-R-DING-0001`）を参照。

**位置づけ**: v0.1.1では『常用字解』を基に初稿・引用を作成した。v0.2.0では筆者が『字通』を主軸に再調査した。現在の主たる辞書的基盤は『字通』である。ただし、字ごとの公開証跡には整備済みのものと未整備のものがあり、引用の文言はv0.1.1初稿の『常用字解』（平凡社）由来を保持する。要約・短い引用・出典明示の形で行う（【字源】）。体系との照応はすべて【象徴的読解】であり、論証ではなく布置の読みである。

本マップは〈白川の字源claim（初稿=『常用字解』・現行再調査の主軸=『字通』。個々の引用・確認資料は資料別に帰属を明示する）〉と〈本体系の象徴的読解〉の二列を分離して示す。前者は一資料の学説の紹介であり、字源の事実の確定ではない。後者は論証ではない読みである。
...
個々の字の照応を見る前に、相関の背骨を示す。形成史上、筆者が最初に字義まで掘り下げて定義した中核五字は**主・客・観・事・鼎**である。白川の字書によれば（v0.2.0で『字通』により再確認）、各字のclaimを一つの祭祀の場面へ布置できる。この時系列は筆者証言に基づく形成史であり、五字が同一語源から派生したという主張ではない。
...
つまり——**主が火を執り、客なる神が訪れ、観によって神意が察され、事として祭りが行われ、鼎がその器として立つ。**体系の最初期の用語一式は、この一つの場面を【象徴的読解】として再構成している。

```mermaid
graph LR
    subgraph gen["字源の層 — 一つの祭祀の場面（白川静の字源claim）"]
        SHU["主：廟で火を執る中心"]
        KYAKU["客：外から降り格る神"]
        KAN["観：鳥占いで神意を察する"]
        KOTO["事：祭りが行われる"]
...
個々の照応が**束になって効く**のは、白川説の各claimを一つの場へ布置できるからである。一字ずつの対応だけでなく、五字の場面上の役割分担が体系の役割分担と重なることは、命名来歴として重要である。ただしこれは論証ではなく布置の読みである（→[鼎の概念解説](../wiki/concepts/kanae.md)／[白川静の字源研究と神事的メタファー](../wiki/comparisons/shirakawa.md)）。鼎の王権・祭祀・刻銘・継承という多層的な歴史は、[鼎銘と鼎の多層用法](kanji/ding/dingming-preqin-han.md)で別に検証する。
...
| 字 | 照応度 | 字源（白川静の記述・v0.1.1初稿） | 本体系の読解（象徴的読解） |
|---|---|---|---|
...
| 体系の語 | 字源の裏づけ（白川の字源claim） | 核となる◎ |
|---|---|---|
| 鼎（Kanae）＝鼎界を成立させる器 | 鼎＝祭器。器＝儀礼の清められたうつわ。貞＝鼎で神意を問う | 器・貞 |
...
以下は『字通』を主軸に再確認した字源・古義のclaimと、体系語のあいだに見いだせる象徴的相関である。定義、公理、原理の証明ではなく、採否理由と形成史を豊かにする研究層として扱う。

| 字・像 | 資料層で確認するclaim | 体系との象徴的相関 |
|---|---|---|
| 魂・鬼と雲気 | 魂・鬼には死者、霊、雲気・遊魂をめぐる古い意味領域が重なる | その豊かな宗教的含意こそ、魂・鬼を原始語にせず、然・則・己・體へ分ける理由になる |
| 衣・糸と縁 | 衣の端、糸の連なり、縁辺をめぐる字形・語義群 | 縁起を、独立物どうしの後付けの接続ではなく、形が生じる条件の連なりとして読む像と響き合う |
| 主と火 | 主の古い字形・用法には灯火やその中心をめぐる説明がある | 主を支配者ではなく、観が発する起点として読む象徴と響き合う |
| 客・各と訪れ | 客・各には外から至るもの、訪れをめぐる意味領域がある | 客鼎観を、他の起点から届き、受け手の内で形成される観として読む像と響き合う |

異説がある字源claimは資料別に保持する。体系側の定義を、単一の字源説から演繹しない。


## 四、注意（公開時）

- 本マップの「本体系の読解」列はすべて【象徴的読解】。字源の記述（【字源】）はv0.1.1で白川静『常用字解』から作成した。v0.2.0の再調査は『字通』を主軸とし、字ごとの再確認状態は照合証跡に即して扱う。白川説は出発点として採用する一学説である（→[思想史的位置づけ](../wiki/positioning.md)）。
